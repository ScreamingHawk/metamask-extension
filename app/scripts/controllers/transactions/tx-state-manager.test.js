...
describe('TransactionStateManager', function () {
  ...
  describe('#getTransactions', function () {
    ...
    it('returns a tx with the requested data', function () {
      ...
      txMetas.forEach((txMeta) => txStateManager.addTransaction(txMeta, noop));
      let searchCriteria;
  ...
  describe('#addTransaction', function () {
    it('adds a tx returned in getTransactions', function () {
      ...
      txStateManager.addTransaction(tx, noop);
      const result = txStateManager.getTransactions();
      assert.ok(Array.isArray(result));
      assert.equal(result.length, 1);
    ...
    it('does not override txs from other networks', function () {
      ...
      txStateManager.addTransaction(tx, noop);
      txStateManager.addTransaction(tx2, noop);
      const result = txStateManager.getTransactions({
    ...
    it('cuts off early txs beyond a limit but does not cut unapproved txs', function () {
      ...
      txStateManager.addTransaction(unconfirmedTx, noop);
      const limit = txStateManager.txHistoryLimit;
